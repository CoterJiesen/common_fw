...
func initLogFile(l *Logger) bool {
	...
	l.fp = f
	l.logger.SetOutput(bufio.NewWriterSize(f, defaultBufSize))
	return true
}
...
func (l *Logger) logf(skip int, lvl Level, tag string, simpleOutput bool, format string, args ...interface{}) bool {
	...
	if simpleOutput {
		...
		funcptr, _, line, ok := runtime.Caller(skip)
		if !ok {
			return false
		}
